...
#if defined(NDEBUG)
# error "crebel cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6aab933020b9c646fba9d0ea7479b6e7c3bb23f1b20be7a92c6aee9a4aee0c0f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // crebel: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "crebel Signed Message:\n";
...
{
    // crebel: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 2.100.000 blocks, which will occur approximately every 2 years
    nSubsidy >>= (nHeight / 2100000); // crebel: 840k blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // crebel: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // crebel: 0,5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // crebel: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // crebel: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Crebelcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x86741d3dc8251514259076cbdbc6f48c3963059e66c1f6a550ed2c61a00cc5d3"));
...
unsigned char pchMessageStart[4] = { 0xd0, 0xe1, 0xf5, 0xec }; // crebel: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("crebelMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("crebelMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static crebelMiner(CWallet *pwallet)
{
    printf("crebelMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("crebel-miner");
    ...
    try { loop {
        ...
        printf("Running crebelMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&crebelMiner, pwallet));
}
